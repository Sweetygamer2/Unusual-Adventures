...
public class NullspaceTeleporter {
	public static Holder<PoiType> poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(Registries.POINT_OF_INTEREST_TYPE, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(UnusualAdventuresModBlocks.NULLSPACE_PORTAL.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register(ResourceLocation.parse("unusual_adventures:nullspace_portal"), poiType);
	...
	public NullspaceTeleporter(ServerLevel level) {
		this.level = level;
	}
	...
	public Optional<BlockUtil.FoundRectangle> createPortal(BlockPos p_77667_, Direction.Axis p_77668_) {
			...
			for (int i3 = -1; i3 < 2; i3++) {
				for (int j3 = 0; j3 < 2; j3++) {
					for (int k3 = -1; k3 < 4; k3++) {
						BlockState blockstate1 = k3 < 0 ? UnusualAdventuresModBlocks.NULL_PLATING.get().defaultBlockState() : Blocks.AIR.defaultBlockState();
		...
		for (int l1 = -1; l1 < 3; l1++) {
			for (int j2 = -1; j2 < 5; j2++) {
				if (l1 == -1 || l1 == 2 || j2 == -1 || j2 == 4) {
					blockpos$mutableblockpos.setWithOffset(blockpos, l1 * direction.getStepX(), j2, l1 * direction.getStepZ());
					BlockState frameBlock;
					if (j2 == 4) {
						frameBlock = UnusualAdventuresModBlocks.NULL_PLATING.get().defaultBlockState();
					} else if (j2 == 3) {
						if (l1 == -1 || l1 == 2) {
							frameBlock = UnusualAdventuresModBlocks.CHARGED_NULL_PLATING.get().defaultBlockState();
						} else {
							frameBlock = UnusualAdventuresModBlocks.NULL_PLATING.get().defaultBlockState();
						}
					} else if (j2 == 2 || j2 == 1) {
						if (l1 == -1 || l1 == 2) {
							frameBlock = UnusualAdventuresModBlocks.CHISELED_NULL_PLATING.get().defaultBlockState();
						} else {
							frameBlock = UnusualAdventuresModBlocks.NULL_PLATING.get().defaultBlockState();
						}
					} else {
					...
					this.level.setBlock(blockpos$mutableblockpos, frameBlock, 3);
				}
			}
		}
		BlockState blockstate = UnusualAdventuresModBlocks.NULLSPACE_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
		...
		return Optional.of(new BlockUtil.FoundRectangle(blockpos.immutable(), 2, 4));
	}
